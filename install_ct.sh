...
if [ "$MODE" = "update" ] && [ -d "$INSTALL_DIR" ]; then
  ...
  # Read existing port from current docker-compose.yml
  EXISTING_PORT=$(grep -oP '"\K[0-9]+(?=:3000")' "${INSTALL_DIR}/docker-compose.yml" 2>/dev/null || echo "")
  [ -n "$EXISTING_PORT" ] && APP_PORT="$EXISTING_PORT" && msg_info "Detected existing port: ${APP_PORT}"
  # Read existing timezone
  EXISTING_TZ=$(grep "TZ=" "${INSTALL_DIR}/docker-compose.yml" 2>/dev/null | head -1 | cut -d= -f2 || echo "")
  [ -n "$EXISTING_TZ" ] && TIMEZONE="$EXISTING_TZ"
